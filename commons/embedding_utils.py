...
def init_vector_store(embedding_model, prompt_doc, is_already_indexed=True):
    ...
    """
    if VECTOR_STORE == 0:
        vector_store = init_pinecone_vector_store(embedding_model, prompt_doc, is_already_indexed)
    elif VECTOR_STORE == 1:
        vector_store = init_chroma_vector_store(embedding_model, prompt_doc, is_already_indexed)
    return vector_store

def init_pinecone_vector_store(embedding_model, prompt_doc, is_already_indexed=True):
    """
